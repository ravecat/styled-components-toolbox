export default (properties, resolution) => {
  try {
    ...
    const argRes = Array.isArray(resolution)
      ? resolution
      : resolution && [resolution];
    const resolutions =
      argRes ||
      process.env.RESPONSIVE.replace(/[\s]/gi, "")
        .split(",")
        .sort((a, b) => a - b);

    if (!resolutions || !resolutions.length || resolutions.some(isNaN)) {
      throw new Error(
        "Responsive value is invalid or not definitely. Must be a number or set of numbers"
      );
    }
    ...
    return (isManyResolutions ? properties : resolutions).reduce(
      (styles, item, index, arr) =>
        styles.concat(
          !index
          ...
          ${properties[index]}
            ...
            : arr[index + 1]
            ? `@media (min-width:${resolutions[index - 1]}px) and (max-width: ${
                resolutions[index]
              }px) {
            ${properties[index]}
          }`
